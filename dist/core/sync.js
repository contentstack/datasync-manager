        ...
        return api_1.get(req).then((response) => {
            ...
            if (syncResponse.items.length) {
                return core_utilities_1.filterItems(syncResponse, config).then(() => {
                    core_utilities_1.formatItems(syncResponse.items, config);
                    const groupedItems = core_utilities_1.groupItems(syncResponse.items);
                    core_utilities_1.markCheckpoint(groupedItems, syncResponse);
